export const PeriodicTableCaseStudy = () => {
  return (
    <article>
      <h1>Periodic Table App - Case Study</h1>

      <h2>Overview</h2>
      <p>
        A mobile-first learning tool for the periodic table, rebuilt from an earlier project after
        studying User Experience Design (UX). The app offers two modes, flashcards with self-rating
        and a multiple choice quiz, designed to suit different learning styles and moments.
      </p>
      ...
      <p>
        ...
        quickly. It worked, but it was rough. Buttons were unstyled, there was no layout structure,
        no progress tracking, and only one way to use it.
      </p>
      <p>
        You can still see the <a href="/apps/PeriodicTableQuiz">original version here</a>.
      </p>
      <p>
        When I started studying User Experience Design (UX), I came back to it. It became the first
        project where I deliberately applied a UX process before writing a single line of code.
      </p>

      <h2>The problem with the original</h2>
      <p>
        Looking at it critically, the original had several issues. Content stacked down the page as
        you clicked through, with no sense of where you were in the sequence. There was no way to
        test yourself, only to reveal answers. The interface gave no feedback and asked nothing of
        the user. It worked as a prototype, but not as a product.
      </p>

      <h2>The process</h2>
      <p>
        I started in Figma rather than code. I defined a clear v1 scope, two modes, mobile first, no
        feature creep, and built a design system first: colour styles, text styles, and a consistent
        component approach. Accessibility was checked at the design stage using the Stark plugin,
        which caught a contrast failure on the primary button colour before it ever reached the
        browser.
      </p>
      <p>
        Eight screens were designed and accessibility-checked before any code was written. This was
        a deliberate change in my working practice, previously I would have reached for the code
        immediately.
      </p>
